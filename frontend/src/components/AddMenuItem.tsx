...
const AddMenuItem = ({onItemAdded} : AddMenuItemProps) => {
  ...
  const handleSubmit = async ({ onItemAdded }: { onItemAdded: () => void }) => {
    if (!name || !price || !image) {
    ...
    try {
      setLoading(true);
      const response = await axios.post(`${restaurantService}/api/item/new`, formData, {
        headers: {
  ...
  return (
    <div className="max-w-md space-y-4 m-auto">
      ...
      <button
        disabled={loading}
        onClick={()=>handleSubmit({ onItemAdded })}
